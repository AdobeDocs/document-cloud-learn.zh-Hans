---
title: 扫描的文档
description: 了解如何使扫描的文档可在Acrobat中访问
feature: Accessibility
role: User
level: Intermediate
topics: Accessibility
jira: KT-18732
source-git-commit: f1b917a7889adf2d372965a7c75ced1aac4cfc4c
workflow-type: tm+mt
source-wordcount: '74'
ht-degree: 0%

---

# 扫描的文档

了解如何使扫描的文档可在Acrobat中访问。 扫描的PDF可能很棘手，但使用正确的工具和技术，您可以使扫描文档可读且易于访问。

>[!NOTE]
>
>本教程的某些功能仅在Acrobat Pro、Premium和Studio中提供。

>[!VIDEO](https://video.tv.adobe.com/v/3476244?captions=chi_hans&quality=12&learn=on&hidetitle=true)

>[!TIP]
>
>此视频是可在Experience League上免费使用的播放列表[PDF辅助功能系列](https://experienceleague.adobe.com/zh-hans/playlists/acrobat-pdf-accessibility)的一部分！