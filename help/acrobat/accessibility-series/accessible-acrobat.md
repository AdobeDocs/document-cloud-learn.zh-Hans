---
title: 在Acrobat中创建可访问的PDF
description: 了解如何从Acrobat开始创建可访问的PDF
feature: Accessibility
role: User
level: Intermediate
topics: Accessibility
jira: KT-18729
source-git-commit: f1b917a7889adf2d372965a7c75ced1aac4cfc4c
workflow-type: tm+mt
source-wordcount: '80'
ht-degree: 0%

---

# 在Acrobat中创建可访问的PDF

了解如何从Acrobat开始创建可访问的PDF。 涵盖的主题包括：基于云的自动添加标签、使用为辅助功能工具做准备以及修复常见问题。

>[!NOTE]
>
>本教程的某些功能仅在Acrobat Pro、Premium和Studio中提供。

>[!VIDEO](https://video.tv.adobe.com/v/3477935?captions=chi_hans&quality=12&learn=on&hidetitle=true)

>[!TIP]
>
>此视频是可在Experience League上免费使用的播放列表[PDF辅助功能系列](https://experienceleague.adobe.com/en/playlists/acrobat-pdf-accessibility)的一部分！
