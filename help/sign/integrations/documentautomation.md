...
<tr>
  <td>
    ...
    <div>
    <a href="documentautomation.md#part1"><strong>第1部分：使用Acrobat Sign在SharePoint中存储已签名的协议</strong></a>
    </div>
  </td>
  <td>
...
1. 搜索Acrobat Sign。
   ...
   ![导航到Power Automate的屏幕快照](assets/documentautomation/automation_1.png)

1. 选择 **将Acrobat Sign已完成的协议保存到SharePoint库**.
...
1. 选择 **模板** （位于Power Automate界面的左侧）。

   ![选择“模板”的屏幕快照](assets/documentautomation/automation_9.png)

1. 搜索“经理批准”。
1. 选择 **请求经理审批选定的文件**.

   ![为选定文件选择“请求经理批准”的屏幕快照](assets/documentautomation/automation_10.png)
...
1. 按如下方式添加输入：
   ...
   **名称**：签名者电子邮箱
   ...
   ![配置工作流的屏幕截图](assets/documentautomation/automation_11.png)
...
1. 向下滚动并查找 **如果是**.
   ...
   ![“如果是”配置的屏幕截图](assets/documentautomation/automation_12.png)

1. 点击 **添加动作** 在 **如果是** 框（不是最底部的框）以添加要发送以请求签名的步骤。

   ![在“如果是”框中添加动作的屏幕快照](assets/documentautomation/automation_13.png)

1. 搜索 **SharePoint获取文件内容** 并选择 **获取文件内容**.
...
1. 配置 **获取文件内容** 如下所示：
   ...
   **站点地址：** 您的SharePoint站点。
   **文件标识符：** 搜索“标识符”，然后从 **获取文件属性** 步骤。
1. 搜索“Adobe”并选择 **Acrobat Sign** 添加其他操作。

   ![搜索菜单的屏幕快照](assets/documentautomation/automation_16.png)

1. 在Acrobat Sign的搜索框中输入“upload”，然后选择 **上传文档并获取文档ID**.
1. 搜索动态变量 **名称** 获取在触发器下选定的项目/文档的名称 **文件名**.
1. 点击 **表达式** 在“变量助手”中的 **文件内容**.

   ![上传文档并获取文档ID屏幕的屏幕截图](assets/documentautomation/automation_17.png)

1. 添加单个撇号，然后重新单击以 **动态内容**，删除您的撇号，选择 **文件内容** 然后单击 **确定**.
...
1. 选择 **从上传的文档创建和协议并发送以供签名**.

   ![搜索创建的屏幕截图](assets/documentautomation/automation_19.png)

1. 配置所需信息：选择 **名称** 从dynamic variable assistant **协议名称**.
选择 **文档ID** 从dynamic variable assistant **文档ID**.
选择 **签名者电子邮箱** 从dynamic variable assistant **参与者电子邮件**.
在以下位置输入“1”： **参与者顺序**.
选择 **签名者** 从下拉列表 **参与者角色**.
...
1. 选择文档，然后选择 **Automate** 以及 **流量** 你刚刚创建了。

   ![选择“自动”菜单和流程的屏幕快照](assets/documentautomation/automation_21.png)

1. 启动工作流以验证连接（仅首次运行工作流）。
1. 在中向审批人输入一条美妙的消息 **消息**.
1. 输入文档签名者的电子邮件 **签名者电子邮箱**.
1. 点击 **运行流**.
...
1. 点击 **+新建** 创建一个名为“Processed Contracts”的新文件夹。
1. 点击 **+新建** 创建一个名为“Old Contracts”的新文件夹。
...
1. 选择 **在SharePoint中将新添加的文件转换为文本可搜索PDF**.
1. 单击 **+** Adobe PDF Tools旁边的符号。
...
1. 单击&#x200B;**继续**。
   ...
   ![单击“继续”位置的屏幕截图](assets/documentautomation/automation_33.png)

   现在，您可以看到工作流的视图，并且需要为您的环境配置它。

1. 选择站点地址字段，并在名为的触发器下选择您使用的SharePoint站点 **在文件夹中创建文件时**.

   ![选择何时在文件夹触发器中创建文件的屏幕截图](assets/documentautomation/automation_34.png)

1. 单击文件夹图标以导航至“Folder ID”（文件夹ID）下的Old Contracts（旧合同）文件夹。

   ![选择“Old Contracts”文件夹的屏幕快照](assets/documentautomation/automation_35.png)
...
1. 点击 **上传** 然后在E04/Flows/中选择GenerateProposal_20210311231623.zip文件夹。

   ![选择文件夹的屏幕截图](assets/documentautomation/automation_41.png)

1. 单击&#x200B;**导入**.

1. 单击操作旁边的扳手图标 **向客户发送提案**.

   ![扳手图标的屏幕快照](assets/documentautomation/automation_42.png)
...
1. 展开 **选定的文件** 作用域操作。
   ...
   将“Site Address”（站点地址）和“Library Name”（库名称）分别更改为SharePoint站点和库的 **获取文件属性**.
将站点地址更改为您位于以下位置的SharePoint站点 **获取文件内容**.

   ![展开的“选定文件”操作的屏幕截图](assets/documentautomation/automation_47.png)
...
1. 在 **Automate** 菜单。
   ...
   ![选择“自动”菜单的屏幕快照](assets/documentautomation/automation_50.png)
...
1. 导航回工作流的末尾。
1. 单击 **+** 符号介于 **合并PDF** 和 **创建文件**.

   ![要选择位置的屏幕截图+符号](assets/documentautomation/automation_54.png)
...
1. 选择 **从查看中PDFProtect**.
1. 使用“动态内容”将“文件名”字段设置为 **合并PDF中的文件名PDF**.

   ![动态内容的屏幕截图](assets/documentautomation/automation_56.png)

   在触发器中，有一个密码字段是启动表单的一部分。 我们可以在这里使用它。

1. 搜索 **密码字段** 使用动态内容，并将它放在“密码”字段中。

   ![搜索密码的屏幕截图](assets/documentautomation/automation_57.png)

1. 使用动态内容将其设置为 **从合并PDFPDF文件内容** 在“文件内容”字段中。
1. 更改 **创建文件** 从ProtectPDF而不是合并PDF获取文件内容。
1. 展开 **创建文件**.
1. 清除文件内容字段。
1. 使用动态内容置入 **PDF文件内容** 自 **从查看中PDFProtect**.
...
1. 选择 **Automate** 以选择您的流程。

   ![从菜单中选择“自动”的屏幕快照](assets/documentautomation/automation_59.png)
